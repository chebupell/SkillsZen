...
export const practiceService = {
  async findCourseIdForBlock(
    blockId: string,
  ): Promise<{ courseId: string; realBlockId: string } | null> {
  ...
  async getPracticeData(
    blockId: string,
    userId: string,
  ): Promise<{ block: unknown; progress: BlockProgress; questions: Task[] } | null> {
    ...
    const questions: Task[] = questionsSnap.docs.map((doc) => {
      const data = doc.data()
      const normalizedAnswers = (data.answers || []).map(
        (
          ans: string | { id?: string | number; text?: string; is_correct?: boolean },
          idx: number,
        ) => {
      ...
      const correctAnsObj = normalizedAnswers.find(
        (a: { is_correct?: boolean; text?: string }) => a.is_correct === true,
      )
